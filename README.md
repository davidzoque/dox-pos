...
| `includes/costs.php` | Los costos y la ganancia: el costo vive en el campo de WooCommerce ("Cost of Goods Sold"), se congela en cada línea del pedido al vender, y cada entrada de mercancía recalcula el promedio ponderado. También la carga de costos desde Excel. |
...
| `includes/class-dox-pos-shipping-weight.php` | El método de envío por peso (`dox_pos_weight`): tramos, precio para lo más pesado y un extra por unidad de peso. Es un método de WooCommerce más, así que también cobra en el checkout de la web. |
...
| `assets/` | `caja.css` y `caja.js` (la caja, sin dependencias) y `ajustes.css` y `ajustes.js` (la página de ajustes: pestañas, vista previa en vivo, comprobación de la dirección, canales ordenables, etiquetas de transportadoras y la prueba de conexión con OpenAI; sin jQuery salvo la biblioteca de medios). |
...
- **La pantalla**: la palabra que va junto al logo ("Caja"; en inglés "POS", porque "Register" suelto en una web se lee como "crear cuenta") y la ruta (`/caja`; `/pos` en inglés).
- **Las ventas**: los canales (los que se entregan en mano no piden ciudad ni envío) y las formas de pago activas, con su nombre y la que sale marcada. A las cinco de fábrica se añaden propias ("Añadir forma de pago": otra cuenta de Nequi, Daviplata, Addi...): cada una lleva una clave fija (`p_` y seis letras, `dox_pos_custom_payments`) que el pedido guarda como `payment_method` (`dox_pos_p_xxxxxx`), así que renombrarla no cambia la historia; cuentan como pagadas al registrar.
...
Las vistas son registrables: la plantilla lanza `dox_pos_history_views` dentro del selector (un botón `data-v="<vista>"`) y `dox_pos_history_panels` después de los paneles (un `div#h-<vista>` oculto), y en el JS `DoxPOS.vistaHistorial({ id, cargar(desde, hasta), pintar(datos) })` dice cómo se carga y se pinta; `cargarHistorial` esconde todos los paneles registrados y enseña el de la vista. Un Excel propio se sirve desde `dox_pos_download` (`/caja/?descargar=<lo que sea>`), que salta cuando el valor no es uno de los del núcleo. El Pro pone así la vista Rendimiento.
...
Quien administra (`manage_woocommerce`) tiene la pestaña Panel, antes de Vender, y la caja abre ahí si el ajuste "Abrir la caja en el Panel" (Ajustes > Caja, `dox_pos_sales[open_panel]`, encendido de fábrica) lo dice: `page.php` lo pasa en `cfg.open_tab` y `abrirDesdeHash(true)` la abre cuando no hay `#`. `GET dox-pos/v1/dashboard` (`includes/dashboard.php`, con el permiso del historial completo) devuelve todo lo que pinta `pintarPanel`: hoy en vivo (las ventas y la caja de `dox_pos_history_sales` y `dox_pos_history_cash`), ayer, la semana y el mes contra los anteriores a la misma altura, los últimos `DOX_POS_DASH_DAYS` días (treinta) para la gráfica, lo cobrado hoy por forma de pago, los pedidos abiertos contados como los entiende la caja (`dox_pos_dashboard_pending`, con lo que hay por cobrar), lo más vendido en treinta días y el inventario en dos cifras (`dox_pos_dashboard_stock`: una consulta a la tabla de búsqueda de WooCommerce que cuenta cada total compartido una sola vez, porque solo suma a quien gestiona el stock). Los días anteriores salen de una sola pasada por las ventas desde el 1 del mes pasado (o desde hace los días que pidan, lo que caiga antes), guardada una hora en el transient `dox_pos_dash_past_<días>` y rehecha al cambiar el día. Cada tarjeta lleva `data-go` con la pestaña que abre, escrita como en el `#` (`historial/ventas/semana`).
...
- **El costo congelado en cada venta.** WooCommerce calcula el costo de cada línea con el costo actual del producto cada vez que el pedido recalcula totales, así que la historia cambiaría al subir un costo. Aquí `woocommerce_new_order_item` guarda en la línea `_dox_pos_unit_cost` (el costo por unidad en ese momento, venga la venta de la caja, de la web o de wp-admin) y el filtro `woocommerce_calculated_order_item_cogs_value` le devuelve a WooCommerce ese congelado, para que las dos cifras coincidan. `dox_pos_order_profit` suma las líneas: la ganancia es lo que pagó la clienta por los productos (con el descuento, sin envío ni impuestos) menos el costo; si a alguna línea le falta el costo, la ganancia de esa venta no se sabe y se cuenta aparte (`no_cost_n`).
- **La pérdida anotada en el pedido.** Lo que una venta costó de más (un envío más caro de lo cobrado, un flete devuelto, un arreglo) se anota desde el detalle del pedido (`POST /orders/{id}/loss` con `amount` y `note`; 0 la quita), va en `_dox_pos_loss` y `_dox_pos_loss_note` con una nota en el historial del pedido, y `dox_pos_order_profit` la resta de la ganancia de esa venta. Quién puede: `dox_pos_can_see_losses` (`manage_woocommerce`, lleve costos la tienda o no); con eso el detalle trae `loss` y `loss_note`, el historial de ventas y la caja del día `loss` (y `loss_n`), y los Excel su columna. El Pro la reparte entre las líneas en Rendimiento igual que el descuento.
- **El costo promedio ponderado.** Cada línea de una entrada de mercancía lleva lo que costó la unidad (solo lo manda quien administra; el rol Caja registra sin costos). Con las unidades ya sumadas, `dox_pos_costs_after_entry` agrupa por quien lleva el costo (el producto, o la talla si tiene uno propio) y hace (lo que había × su costo + lo que entra × lo que costó) / (lo que había + lo que entra); sin existencias o sin costo, el costo pasa a ser el de esa compra. La entrada guarda el costo de cada línea y el que había (`cost`, `cost_before` en `items`) y su total en la columna `cost`; el kardex guarda `unit_cost` en cada fila (el de compra en una entrada, el del producto en lo demás) por el contexto `dox_pos_stock_context( ..., $costs )`.
- **La carga de golpe.** `POST /costs/import` recibe el Excel del inventario con la columna Costo llena: `dox_pos_xlsx_read` lee la primera hoja (ZipArchive y SimpleXML, sin bibliotecas; entiende textos en la celda y en la tabla compartida) y `dox_pos_costs_import` busca cada código con `wc_get_product_id_by_sku`: una fila con código pone el costo a esa talla; una fila de producto (Ref. sin código), a todo el producto.

Quién lo ve: `dox_pos_can_see_costs` (costos encendidos y `manage_woocommerce`). Con eso, la API manda `cost` en las variaciones del buscador, el formulario de producto trae `cost` (`''` si las tallas cuestan distinto, con `cost_min` y `cost_max`), el historial de ventas y la caja del día llevan `cost`, `profit`, `margin` y `no_cost_n`, el kardex `unit_cost` y `value`, el detalle del pedido la ganancia y el costo de cada línea, y los Excel sus columnas. Sin permiso, nada de eso viaja.
...
- **En el historial**: `dox_pos_history_views` y `dox_pos_history_panels` (una vista más) y
  `DoxPOS.vistaHistorial`; y `dox_pos_download` para un Excel propio (ver "El historial y el kardex").
...
## Los costos de envío, desde la caja

La caja sigue sin tarifas propias: lo que se arma desde el engranaje (arriba a la derecha, solo con `manage_woocommerce`; `cfg.shipping_setup`) son las **zonas y los métodos de envío de WooCommerce**, así que el checkout de la web y la caja cobran lo mismo. La pantalla es una hoja (`#sheet`): en el teléfono sube desde abajo y se baja arrastrándola por la cabecera (sigue al dedo y al soltar decide por la velocidad, `armarHoja`); en pantalla ancha es una tarjeta centrada. Dentro lleva una pila de vistas (`empujar`, `volver`): la lista de zonas, cómo se cobra, el costo y la zona. Ojo: cada vista se pinta **antes** de colgarse del documento, así que dentro de `pintar(el)` todo se busca con `el.querySelector`, nunca con `$()`.

- **Lo que sabe editar**: `flat_rate` (precio fijo, si el costo es un número), `free_shipping` (siempre, o desde un monto), `local_pickup` y `dox_pos_weight`. Un precio con fórmula, un envío gratis con cupón o el método de otro plugin se enseñan tal cual (`editable: false`, con `why`: `formula`, `coupon` u `other`) y solo se encienden o se apagan; tampoco se borran desde ahí, porque la caja no sabría volver a armarlos.
- **Envío por peso** (`Dox_POS_Shipping_Weight`): tramos "hasta X, cuesta Y" guardados uno por línea (`1 | 6`) en el ajuste `tiers` de la instancia, más `over_cost` (lo más pesado que el último tramo; vacío = vale el último tramo) y `over_extra` (por cada unidad de peso de más, redondeando hacia arriba). El peso del paquete es la suma de `get_weight()` por unidades, en la unidad de la tienda; un producto sin peso cuenta cero. En WooCommerce > Ajustes > Envío sale con los mismos datos.
- **Zonas**: todo el país de la tienda (`scope: country`) o algunas de sus regiones (`states`); "todo lo demás" es la zona 0 de WooCommerce (`rest`). Las que cubren otros países o códigos postales (`custom`) solo se renombran aquí. WooCommerce cobra **la primera zona que encaja**, así que la zona guardada se coloca antes que cualquiera más amplia que ella (el país entero para una de regiones; varios países, un continente o una zona sin lugares, que encaja con todo, para cualquiera) y después de las más estrechas (`dox_pos_shipping_zone_relation` dice qué es cada una y `dox_pos_shipping_place_zone` la coloca); si ya está en un sitio que vale no se mueve, y las demás conservan su orden. Un lugar no puede estar en dos zonas. El nombre que la caja pone sola a una zona (`dox_pos_shipping_zone_auto_name`) se rehace si cambian sus lugares; el escrito a mano se respeta. Una zona sin costos deja a sus lugares sin envío (WooCommerce no pasa a la siguiente) y la lista lo avisa.
- **Rutas** (todas piden `manage_woocommerce`): `GET /shipping/setup`, `POST /shipping/zones[/{zona}]`, `DELETE /shipping/zones/{zona}`, `POST /shipping/zones/{zona}/rates[/{instancia}]` y `DELETE` de lo mismo. Cada escritura devuelve la pantalla entera y renueva la versión de los transientes de envío (`WC_Cache_Helper::get_transient_version( 'shipping', true )`).
- **En la venta**: si el país usa código postal (`dox_pos_postcode_label()`: lo exige su `locale` de WooCommerce; en Colombia no), la dirección lo pide, el pedido lo guarda y viaja en el paquete para que encajen las zonas por código postal. Si la tienda no cobra nada a ese destino, la venta lo dice (`st.sinTarifa`) y el importe se sigue pudiendo escribir a mano.
- **Al desinstalar** no se toca nada de esto: las zonas y sus métodos son de WooCommerce. Una instancia de `dox_pos_weight` sin el plugin queda inerte (WooCommerce ignora los métodos que no conoce) y vuelve si se instala otra vez.
...
- **Existencias**: se comprueban en el servidor al registrar, no con lo que el teléfono vio antes, y descontando lo que la tienda tiene retenido en pagos en curso (`wc_get_held_stock_quantity`). Si no alcanza: "De X quedan 0, no 1". Las tallas que comparten el total del producto (`get_manage_stock()` devuelve `parent`) se suman antes de comprobar (`dox_pos_shared_stock_problem`, por `get_stock_managed_by_id`): de a una pasaban las dos. En la caja, el buscador manda `shared` y `parent` por variación, y lo libre de una talla descuenta lo que el pedido lleva de todas sus hermanas; la fila del producto cuenta ese total una sola vez.
...
- **Dos tallas de la misma bolsa en el mismo instante**: las hermanas de una bolsa por color se mueven por diferencia, no copiando el número (ver "Unidades compartidas por color"), así las dos ventas restan.
...
- **Categorías**: todas menos la de "sin categoría", agrupadas por la de arriba. Se enseñan las que ya tienen productos, las elegidas y las creadas en la sesión (`categoriasVisibles`); las vacías quedan detrás de "Más categorías…", y si ninguna tiene productos (una tienda que empieza) salen todas. "Nueva categoría…" abre una fila con el nombre y dentro de cuál va (por defecto, de ninguna): `POST /products/categories` la crea al momento (`dox_pos_create_category`; si ya hay una con ese nombre en ese sitio, devuelve esa) y queda elegida. Para cada una se mira cómo están hechos sus productos: el prefijo de código más usado y el conjunto de tallas más repetido. Elegir la primera categoría marca esas tallas y pide el siguiente código libre (`GET /products/sku?prefix=VE`); escribir otro código lo comprueba mientras se escribe (`?sku=`).
...
- **Peso y medidas** (`package`: `weight`, `length`, `width`, `height`, en las unidades de la tienda, `cfg.units`): van en el producto y cada talla los hereda. `dox_pos_measure` acepta la coma decimal y guarda con punto. Los paquetes que más se repiten en la tienda (`dox_pos_common_packages`, una consulta agrupada sobre `_weight`, `_length`, `_width` y `_height`) salen como botones para ponerlos de un toque. Al editar se mandan **solo las medidas que cambiaron** (`paqueteCambios`), y el servidor toca solo las que llegan. Lo que las tallas no llevan todas igual (alguien afinó una en WooCommerce) se mira medida por medida: esa medida llega vacía y apuntada en `mixed_fields` (con `mixed` en true), y las que sí comparten se enseñan. Vacío no toca nada; lo que se escriba va al producto y las tallas sueltan la suya para heredarlo. Así escribir el peso de un producto cuyas tallas pesan distinto no le borra el largo, el ancho y el alto. Las tres medidas se exigen juntas solo si se tocó alguna. Si alguna zona cobra por peso (`weight_matters`), el repaso avisa de un producto sin peso. Ojo al probarlo: desde WooCommerce 10.5 los productos se guardan en memoria durante la petición (`product_instance_caching`), así que en la misma petición una variación puede seguir enseñando el peso viejo del padre; en la siguiente ya no.
...
## Unidades compartidas por color

WooCommerce solo sabe llevar las existencias de un producto de tallas de dos formas: un total del producto que las tallas heredan (`get_manage_stock()` devuelve `'parent'`) o las de cada talla por su cuenta. Un producto de un solo color usa el total del producto, como siempre. Con varios colores, "Comparte unidades" va por color y lo resuelve `includes/pools.php` encima de WooCommerce: cada talla de la bolsa lleva sus propias existencias con el mismo número y el meta `_dox_pos_pool` (el slug de su color); `dox_pos_pool_remember` (en `woocommerce_variation_before_set_stock`) apunta lo que la talla tenía en la base y `dox_pos_pool_sync` (en `woocommerce_variation_set_stock`) mueve a sus hermanas esa diferencia cuando una cambia (una venta, una entrada, un cambio a mano), no el número entero, así dos ventas de tallas distintas de la misma bolsa en el mismo instante restan las dos (sin dato de antes, o mientras se forman o se mueven las bolsas, copia el número), sin volver a entrar y sin que el kardex apunte a las hermanas (`dox_pos_stock_quiet`); lo que queda en la bolsa es el menor de sus tallas (`dox_pos_pool_stock`, por si alguna se quedó atrás); y `dox_pos_pool_reserved_query` (filtro `woocommerce_query_for_reserved_stock`) hace que lo retenido en los pagos en curso sume toda la bolsa, así el checkout de la web (y la reserva de la caja, que usa el mismo mecanismo) no deja que dos personas se lleven dos tallas cuando queda una.
