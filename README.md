...
- **Zonas**: todo el país de la tienda (`scope: country`) o algunas de sus regiones (`states`); "todo lo demás" es la zona 0 de WooCommerce (`rest`). Las que cubren otros países o códigos postales (`custom`) solo se renombran aquí. WooCommerce cobra **la primera zona que encaja**, así que una zona de regiones se coloca antes que la del país entero sin tocar el orden de las demás (`dox_pos_shipping_place_zone`), y un lugar no puede estar en dos zonas. Una zona sin costos deja a sus lugares sin envío (WooCommerce no pasa a la siguiente) y la lista lo avisa.
...
- **Peso y medidas** (`package`: `weight`, `length`, `width`, `height`, en las unidades de la tienda, `cfg.units`): van en el producto y cada talla los hereda. `dox_pos_measure` acepta la coma decimal y guarda con punto. Los paquetes que más se repiten en la tienda (`dox_pos_common_packages`, una consulta agrupada sobre `_weight`, `_length`, `_width` y `_height`) salen como botones para ponerlos de un toque. Al editar, el paquete solo se manda si cambió; si las tallas no llevan todas lo mismo (alguien afinó una en WooCommerce) llega `mixed` y el formulario lo deja vacío: vacío no toca nada, y lo que se escriba va al producto y las tallas sueltan el suyo para heredarlo. Si alguna zona cobra por peso (`weight_matters`), el repaso avisa de un producto sin peso. Ojo al probarlo: desde WooCommerce 10.5 los productos se guardan en memoria durante la petición (`product_instance_caching`), así que en la misma petición una variación puede seguir enseñando el peso viejo del padre; en la siguiente ya no.
